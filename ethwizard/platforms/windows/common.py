...
def is_adx_supported(base_directory, log):
    # Detect if ADX instructions set is support on this CPU

    base_directory = Path(base_directory)

    log.info('Detecting ADX instructions set support for this CPU using Sysinternals Coreinfo...')

    download_path = base_directory.joinpath('downloads')
    download_path.mkdir(parents=True, exist_ok=True)

    # Download the Coreinfo archive
    file_name = urlparse(COREINFO_DOWNLOAD_URL).path.split('/')[-1]
    download_archive_path = download_path.joinpath(file_name)

    if download_archive_path.is_file():
        download_archive_path.unlink()

    try:
        with open(download_archive_path, 'wb') as binary_file:
            log.info('Downloading Coreinfo archive...')
            with httpx.stream('GET', COREINFO_DOWNLOAD_URL, follow_redirects=True) as http_stream:
                if http_stream.status_code != 200:
                    log.error(f'Cannot download Coreinfo archive {COREINFO_DOWNLOAD_URL}.\n'
                        f'Unexpected status code {http_stream.status_code}')
                    return False
                for data in http_stream.iter_bytes():
                    binary_file.write(data)
    except httpx.RequestError as exception:
        log.error(f'Exception while downloading Coreinfo archive. Exception {exception}')
        return False

    # Extracting the archive
    extract_directory = download_path.joinpath('Coreinfo')
    ...
    process_result = subprocess.run([
        'tar', 'xvf', download_archive_path, '--directory', extract_directory])
    
    if process_result.returncode != 0:
        log.error(f'Unexpected return code from tar {process_result.returncode}.')
        return False
    
    # Removing download leftover
    os.unlink(download_archive_path)
    
    coreinfo_binary = extract_directory.joinpath('Coreinfo64.exe')

    if not coreinfo_binary.is_file():
        log.error('Could not find Coreinfo64.exe in Coreinfo archive.')
        return False
    
    # Find CPU features by running Coreinfo
    process_result = subprocess.run([
        str(coreinfo_binary), '/accepteula', '-f'
        ], capture_output=True, text=True)
    
    if process_result.returncode != 0:
        log.error(f'Unexpected return code from Coreinfo {process_result.returncode}.')
        return False
