...
def select_network(log):
    ...
    network_queue_info = {
        ...
        NETWORK_HOODI: unknown_joining_queue
    }
...
'''
        ...
        values=[
            ...
            (NETWORK_HOODI, f'{NETWORK_LABEL[NETWORK_HOODI]} {network_queue_info[NETWORK_HOODI]}')
        ],
        ok_text='Use this',
        cancel_text='Quit'
    ).run()
...
Make sure you have <b>control</b> over this address. This is where the transaction
...
def select_withdrawal_address(log):
    # Prompt the user for a withdrawal address
    
    result = button_dialog(
        title='Withdrawal address',
        text=(
f'''
When creating your validator keys, you have the option to add a
withdrawal address. We suggest you add one, that you make sure you have
full control over that address and that you will have full control over
that address in the future. If you do not add a withdrawal address now,
you will be able to add one later with your mnemonic.

The withdrawal address is where your deposit and all your validator
rewards will go into regularly and when you exit. It cannot be changed.

Do you want to use a withdrawal address?
'''     ),
        buttons=[
            ('Yes', 1),
            ('No', 2),
    ...
    if result == 2:
        log.info('No withdrawal address used.')
        return ''
...
a valid Ethereum address.</style>'''
        ...
        entered_address = input_dialog(
            ...
            text=(HTML(
f'''
Please enter an Ethereum address to be used as your withdrawal address.
Make sure you have <b>control</b> over this address. This is where your deposit
and all your validator rewards will go into regularly and when you exit.
...
'''             ))).run()
    ...
    log.info(f'Using withdrawal address {entered_address}')
...
def get_mevboost_latest_version(log):
    ...
    if response.status_code != 200:
        log.error(f'HTTP error while getting the latest stable version for MEV-Boost. '
            f'Status code {response.status_code}')
        return UNKNOWN_VALUE
    ...
    if 'tag_name' not in release_json or not isinstance(release_json['tag_name'], str):
        log.error(f'Unable to find tag name in Github response while getting the latest stable '
            f'version for MEV-Boost.')
        return UNKNOWN_VALUE
    
    tag_name = release_json['tag_name']
    result = re.search(r'v?(?P<version>.+)', tag_name)
    if not result:
        log.error(f'Cannot parse tag name {tag_name} for MEV-Boost version.')
        return UNKNOWN_VALUE
